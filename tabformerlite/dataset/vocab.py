...
class AttrDict(dict):
    """
    Defines a dictionary type for the attributes.
    """
...
class Vocabulary:
    """
    This class holds the vocabulary used for a TabDataset.
    The vocabulary is separated for each column of the Dataset (local vocabulary),
    but a global vocabulary index is still created, as class NLP vocabulary would.
    """

    def __init__(self, adap_thres=10000, target_column_name=None):
        """
    ...
    def set_id(self, token, field_name, return_local=False):
        """
        Set the id of a token in the vocabulary.
        """
    ...
    def get_id(self, token, field_name="", special_token=False, return_local=False):
        """
        Redefine get_id to return the UNK token
        when an unknown token is encountered.
        """

        if special_token:
            field_name = self.special_field_tag

        # Default value is the UNK token
        unk_token = self.token2id[self.special_field_tag][self.unk_token]
        global_id, local_id = self.token2id[field_name].get(token, unk_token)
    ...
    def set_field_keys(self, keys):
        """
        Create the keys for the fields (columns) vocabulary keys.
        """
        for key in keys:
    ...
    def get_field_ids(self, field_name, return_local=False):
        """
        Given a field name, return the ids of the field in the vocabulary.
        """
    ...
    def get_from_global_ids(self, global_ids, what_to_get="local_ids"):
        """
        Find a token given its global id.
        """
        ...
        def map_global_ids_to_tokens(gid):
            return (
                f"{self.id2token[gid][1]}_{self.id2token[gid][0]}"
                if gid != -100
                else "-"
            )
        ...
        if what_to_get == "local_ids":
            ...
            raise ValueError(
                "Only 'local_ids' or 'tokens' can be passed as value of the 'what_to_get' parameter."
            )

    def save_vocab(self, fname):
        """
        Save the vocabulary to the disk
        """
        self.filename = fname
    ...
    def get_field_keys(self, remove_target=True, ignore_special=False):
        """
        Return the field (columns) names of the vocabulary.
        """
    ...
    def get_special_tokens(self):
        """
        Return the special tokens of the vocabulary.
        """
        special_tokens_map = {}
        keys = [
            "unk_token",
